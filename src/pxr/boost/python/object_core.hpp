...
# include "./detail/is_xxx.hpp"
# include "./detail/string_literal.hpp"
# include "./detail/def_helper_fwd.hpp"
...
// attribute/item access or addition.
  ...
  //
  PXR_BOOST_PYTHON_IS_XXX_DEF(proxy, pxr::boost::python::api::proxy, 1)
...
# endif
  ...
  struct is_derived
    : pxr::boost::python::detail::is_convertible<
...
# define PXR_BOOST_PYTHON_FORWARD_OBJECT_CONSTRUCTORS(derived, base)               \
    inline explicit derived(::pxr::boost::python::detail::borrowed_reference p)     \
        : base(p) {}                                                           \
    inline explicit derived(::pxr::boost::python::detail::new_reference p)          \
        : base(p) {}                                                           \
    inline explicit derived(::pxr::boost::python::detail::new_non_null_reference p) \
        : base(p) {}
  ...
  struct object_initializer_impl<false, true>
  {
      ...
      get(T const& x, U)
      {
          return python::incref(get_managed_object(x, pxr::boost::python::tag));
      }
  };
...
{ 
  ...
  return pxr::boost::python::detail::args_proxy(x);
} 
...
{ 
  ...
  PyObject *result = PyObject_Call(get_managed_object(self, pxr::boost::python::tag),
                                   args.operator object().ptr(), 
                                   0); 
  return object(pxr::boost::python::detail::new_reference(result));
