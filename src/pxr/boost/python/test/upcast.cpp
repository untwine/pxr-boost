...
int main()
{
    ...
    assert(&Py_REFCNT(pxr::boost::python::upcast<PyObject>(&o)) == &Py_REFCNT(&o));
    assert(&Py_REFCNT(pxr::boost::python::upcast<PyObject>(&y)) == &Py_REFCNT(&y));
#else
    assert(&pxr::boost::python::upcast<PyObject>(&o)->ob_type == &o.ob_base.ob_base.ob_type);
    assert(&pxr::boost::python::upcast<PyObject>(&y)->ob_type == &y.ob_type);
#endif
