...
// pxr::boost::python::make_tuple below are for gcc 4.4 -std=c++0x compatibility
// (Intel C++ 10 and 11 with -std=c++0x don't need the full qualification).

#include "../../converter/registrations.hpp"
#include "../../object/function_doc_signature.hpp"
#include "../../errors.hpp"
#include "../../str.hpp"
#include "../../args.hpp"
#include "../../tuple.hpp"
...
namespace pxr { namespace boost { namespace python { namespace objects {
    ...
    str function_doc_signature_generator::pretty_signature(function const *f, size_t n_overloads,  bool cpp_types )
    {
        ...
        if (cpp_types )
        {
            return str(
                "%s %s(%s%s%s%s)"
                % pxr::boost::python::make_tuple // workaround, see top
                ( ret_type
                    , f->m_name
